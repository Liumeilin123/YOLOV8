...
from ultralytics.nn.fpn.HSFPN import ChannelAttention_HSFPN , Multiply,Add
...
from .modules.CLLAhead import CLLAHead
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect,CLLAHead)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect,CLLAHead)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in forward(torch.zeros(2, ch, s, s).to(torch.device('cuda')))])  # forward
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t =type(m)
        if t in (Detect,Segment,CLLAHead):
            m.inplace = inplace
...
def attempt_load_one_weight(weight, device=None, inplace=True, fuse=False):
    ...
    for m in model.modules():
        t =type(m)
        if t in (nn.LeakyReLU,nn.ReLU,nn.ReLU6,nn.SiLU,Detect,Segment,CLLAHead):
            m.inplace = inplace
        elif t is nn.Upsample and not hasattr(m, "recompute_scale_factor"):
            m.recompute_scale_factor = None  # torch 1.11.0 compatibility
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3,C2f_DCN,C2f_DCNv4):
        ...
        elif m in (Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, CLLAHead):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, CLLAHead)):
                #WorldDetect, 
                return "detect"
